...
<template>
  <PagePermission>
    ...
    <nav class="max-sm:border-y max-sm:shadow-sm">
      <NavTabs class="hidden sm:flex">
        {{#each @controller.types as |t|}}
          <Item>
            <A
              class={{if (eq t @controller.type) "active"}}
              {{on "click" (fn @controller.updateParam "type" t)}}
            >
              By
              {{t}}
            </A>
    ...
    <div class="tab-content grid h-full pt-3">
      <NoMobileMessage
        class="{{if
            (not (includes @controller.type @controller.mobileTypes))
            'sm:hidden'
            'hidden'
          }}"
      />
      <StatisticList
        ...
        @missingParams={{@controller.missingParams}}
        @ordering={{@controller.ordering}}
        @onOrderingChange={{fn @controller.updateParam "ordering"}}
      />
    </div>
  </PagePermission>
</template>
