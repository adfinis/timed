...
<template>{{#let @controller.credit.lastSuccessful.value as |credit|}}
  {{#if (or (and credit.isNew (can "create absence-credit")) (and (not credit.isNew) (can "edit absence-credit")))}}
    <div class="grid md:grid-cols-3">
      <div class="grid-cell"></div>
      <ValidatedForm @model={{changeset credit @controller.AbsenceCreditValidations}} @on-submit={{perform @controller.save}} as |f|>
        ...
        <Card as |c|>
          ...
          <c.block class="grid gap-2">
            <f.input @label="Type" @name="absenceType" as |fi|>
              <div class="btn-group [&>*]:w-full">
                {{#each @controller.absenceTypes.lastSuccessful.value as |type|}}
                  <button class="btn
                      {{if (eq fi.value.id type.id) "btn-primary" "btn-default"}}" type="button" {{on "click" (fn fi.update type)}}>{{type.name}}</button>
                {{/each}}
              </div>
            </f.input>
            <div>
            ...
            <f.input class="rounded" @type="number" @label="Days" @name="days" />
            <f.input class="rounded" @type="text" @label="Comment" @name="comment" />
          ...
          <c.footer class="flex justify-between">
            ...
            <div class="flex gap-2">
              {{#unless credit.isNew}}
                <button type="button" data-test-absence-credit-delete class="btn btn-danger {{if @controller.delete.isRunning "loading"}}" {{on "click" (perform @controller.delete credit)}}>Delete</button>
              {{/unless}}
