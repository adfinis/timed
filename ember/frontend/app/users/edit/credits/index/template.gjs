...
<template><div class="grid gap-2 sm:grid-cols-2">
  <div class="year-select col-span-full flex justify-end gap-2">
    {{#if @controller.allowTransfer}}
      <button type="button" class="btn btn-success" {{on "click" (perform @controller.transfer)}}>Transfer</button>
    {{/if}}
    ...
    {{#let @controller.years.lastSuccessful.value as |years|}}
      ...
      <select id="credit-year-select" class="form-control rounded sm:!w-auto" {{on "change" (pick "target.value" @controller.fetchData)}}>
        {{#each years as |y|}}
          <option value={{y}} selected={{eq y @controller.year}}>{{y}}</option>
        {{/each}}
  ...
  <Card class="grid grid-rows-[min-content,1fr,min-content]" data-test-absence-credits as |c|>
    ...
    {{#let (can "create absence-credit") as |canCreate|}}
      <c.block>
        ...
        {{else}}
          {{#let @controller.absenceCredits.lastSuccessful.value as |absenceCredits|}}
            {{#if absenceCredits}}
              <Table class="table--striped table">
                ...
                <tbody>
                  {{#each absenceCredits as |absenceCredit|}}
                    <Tr @striped={{true}} @hover={{can "edit absence-credit"}} @last={{true}} role="link" {{on "click" (perform @controller.editAbsenceCredit absenceCredit.id)}}>
      ...
      <c.footer>
        {{#if canCreate}}
          <LinkTo @route="users.edit.credits.absence-credits.new" class="btn btn-primary block w-min whitespace-nowrap">Add new credit</LinkTo>
        {{/if}}
      </c.footer>
    {{/let}}
  </Card>

  <Card class="grid grid-rows-[min-content,1fr,min-content]" data-test-overtime-credits as |c|>
    {{#let (can "create overtime-credit") as |canCreate|}}
      ...
      {{else}}
        {{#let @controller.overtimeCredits.lastSuccessful.value as |overtimeCredits|}}
          <c.block>
            {{#if overtimeCredits}}
              <Table class="table--striped table">
                <Thead>
                ...
                <tbody>
                  {{#each overtimeCredits as |overtimeCredit|}}
                    <Tr @striped={{true}} @hover={{can "edit overtime-credit"}} @last={{true}} role="link" {{on "click" (perform @controller.editOvertimeCredit overtimeCredit.id)}}>
                      <Td>{{luxonFormat overtimeCredit.date "dd.MM.yyyy"}}</Td>
                      <Td>{{humanizeDuration overtimeCredit.duration false}}</Td>
      ...
      <c.footer>
        {{#if canCreate}}
          <LinkTo @route="users.edit.credits.overtime-credits.new" class="btn btn-primary w-min whitespace-nowrap">Add new credit</LinkTo>
        {{/if}}
