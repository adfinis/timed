...
<template><div class="grid-rows-min grid gap-2 md:grid-cols-2 lg:grid-cols-3 [&>*]:h-min">
      ...
      <Table class="user-general-info">
        <tbody class="[&>tr>td:last-child]:text-right [&>tr>td]:py-1">
          ...
          {{#if @controller.model.activeEmployment}}
            ...
            <tr>
              <td>Contract type:</td>
              <td>{{if @controller.model.activeEmployment.isExternal "External" "Internal"}}</td>
            </tr>
            <tr>
              <td>Percentage:</td>
            ...
            <tr>
              <td>Worktime:</td>
              <td>{{humanizeDuration @controller.model.activeEmployment.worktimePerDay false}}</td>
            </tr>
          {{/if}}
        </tbody>
      </Table>
              ...
              <tbody>
                {{#each employments as |employment|}}
                  <Tr @striped={{true}} @last={{true}}>
                    ...
                    <Td>{{if employment.end (luxonFormat employment.end "dd.MM.yyyy") "-"}}</Td>
                  </Tr>
                {{/each}}
              </tbody>
            </Table>
