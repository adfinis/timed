...
module("Integration | Component | async list", function (hooks) {
  ...
  test("yields list on success", async function (assert) {
    ...
    await render(hbs`<AsyncList @data={{this.data}} as |section data|>
  {{#if (eq section "body")}}
    {{#each data as |d|}}
  ...
  test("yields empty section", async function (assert) {
    ...
    await render(hbs`<AsyncList @data={{this.data}} as |section|>
  {{#if (eq section "empty")}}
  ...
  test("shows loading icon", async function (assert) {
    ...
    await render(hbs`<AsyncList @data={{this.data}} />`);
